document.addEventListener("DOMContentLoaded", () => {
  const tbody = document.getElementById("delivery-edit-table-body");
  const btnAdd = document.getElementById("delivery-edit-btn-add");
  const totalEl = document.getElementById("delivery-edit-total-value");
  ...
  const addRow = (initialProductId = "", initialQty = 0) => {
    ...
    tr.innerHTML = `
      ...
      <td>
        ...
        <select class="delivery-edit-row-input text-left product-select" style="width: 100%;">
          <option value="">Chọn hàng hóa...</option>
          ${options}
        </select>
      </td>
      <td><input type="text" class="delivery-edit-row-input product-code" readonly></td>
      <td><input type="text" class="delivery-edit-row-input product-unit" readonly></td>
      <td><input type="text" class="delivery-edit-row-input product-stock" readonly></td>
      <td><input type="text" class="delivery-edit-row-input product-price unit-price readonly" readonly></td>
      <td><input type="number" class="delivery-edit-row-input quantity product-qty" min="0" value="${initialQty}" ${!initialProductId ? 'disabled' : ''}></td>
      <td><input type="text" class="delivery-edit-row-input line-total readonly" value="0" readonly></td>
      <td style="text-align: center;">
        <button class="delivery-edit-delete-btn" type="button" style="background:none; border:none; color:#e53c2b; cursor:pointer; padding: 4px 8px;">
    ...
    tr.querySelector(".delivery-edit-delete-btn").addEventListener("click", () => {
      tr.remove();
      updateRowNumbers();
      calculateTotal();
    });
